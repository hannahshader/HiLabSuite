...
# @Last Modified time: 2023-07-07 15:21:02
            ...
            for key, value in utterance_map.items():
                ...
                for utt_dict in value:
                    ...
                    if self.overlaps == True:
                        if self.turn_criteria_overlaps(utt_dict, prev_utt):
    ...
    def print_all_rows_text(self, format_markers, outfile: IO[str], formatter) -> None:
        ...
        for index in range(len(self.list)):
            if index != 0:
                if self.overlaps:
                    turn_change = (
                        self.list[index].flexible_info
                        != self.list[index - 1].flexible_info
                    )
                else:
                    turn_change = (
                        self.list[index].speaker != self.list[index - 1].speaker
                    )
